...
public static class MorphineItemSystem
{
    ...
    public const string Description =
        "强效阿片类止痛剂。注射后迅速消除疼痛与震颤，让伤员能在重伤下继续行动。\n\n" +
        "<color=#4fc3f7>效果：300秒内强力止疼，消除疼痛性震颤与视觉红化，精神状态提升。</color>\n" +
        "<color=#ff6666>副作用：立即扣除饱食度10、水分15，药效消退后疼痛可能复发。</color>";
...
public sealed class MorphineEffectController : MonoBehaviour
{
    ...
    private const float OpiateDose = 35f;           // 注入到 Painkillers.opiateAmount 的剂量
    ...
    public void ActivateOrRefresh()
    {
        ...
        StimBuffIndicator.ShowBuff(
            ...
            _remaining,
            DurationSeconds,
            new Color(0.31f, 0.76f, 0.97f)); // 青蓝色（止疼）
    }
